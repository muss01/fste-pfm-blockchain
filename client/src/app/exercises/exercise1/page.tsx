...
export default function Exercise1Page() {
    ...
    useEffect(() => {
        const fetchBlockchainInfo = async () => {
            try {
                const contractData = (await import('../../contracts/Exercise1.json')) as ContractData;
    ...
    const handleAdd = async () => {
        try {
            ...
            if (!deployedNetwork) {
                throw new Error('Contrat non déployé sur le réseau actuel');
            }

            const contract = new web3.eth.Contract(contractData.abi, deployedNetwork.address);
            const accounts = await web3.eth.getAccounts();

            const gasEstimate = await contract.methods.add(a, b).estimateGas({ from: accounts[0] });
            const tx = await contract.methods.add(a, b).send({ from: accounts[0], gas: gasEstimate });

            const sum = await contract.methods.add(a, b).call();
    ...
    return (
        <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 sm:p-20 bg-gray-50">
            <main className="flex flex-col gap-10 row-start-2 items-center text-center w-full max-w-2xl">
                ...
                <Link
                    href="/"
                    className="inline-flex items-center gap-2 self-start bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-100 px-4 py-2 rounded-full text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                >
                    ← Retour à l’accueil
                </Link>

                <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-6 text-left max-w-4xl">
                    <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">L’Addition</h1>

                    <div className="space-y-4">
                        <input type="number" placeholder="Premier nombre" value={a} onChange={e => setA(e.target.value)} className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" disabled={loading} />
                        <input type="number" placeholder="Deuxième nombre" value={b} onChange={e => setB(e.target.value)} className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" disabled={loading} />
                        <button onClick={handleAdd} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400">
                            {loading ? 'Traitement...' : 'Additionner les nombres'}
                        </button>
                ...
                {blockchainInfo.latestBlock && (
                    <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-8 text-left max-w-4xl">
                        ...
                        <div className="bg-gray-50 dark:bg-[#1e1e1e] p-6 rounded-xl space-y-4 border border-gray-200 dark:border-gray-700">
                            <h3 className="text-lg font-semibold text-purple-700 dark:text-purple-400 border-b border-purple-400 dark:border-purple-600 pb-2">
                        ...
                        {transactionInfo && (
                            <div className="bg-gray-100 dark:bg-[#222] p-6 rounded-xl space-y-4 border border-gray-300 dark:border-gray-700">
                                <h3 className="text-lg font-semibold text-green-700 dark:text-green-400 border-b border-green-400 dark:border-green-600 pb-2">Transaction #1</h3>
                                <p><strong>Expéditeur :</strong> {transactionInfo.from}</p>
                                <p><strong>Destinataire :</strong> {transactionInfo.to}</p>
