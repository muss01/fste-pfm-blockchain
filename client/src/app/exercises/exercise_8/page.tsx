...
export default function Exercise8Page() {
    ...
    useEffect(() => {
        const fetchBlockchainInfo = async () => {
            try {
                const contractData = (await import('../../contracts/Exercise8.json')) as ContractData;
    ...
    const handleSendPayment = async () => {
        try {
            ...
            const tx = await contractInstance.methods.receivePayment().send({
            ...
            setResult(`✅ Paiement de ${value} ETH envoyé avec succès.`);
        } catch (err) {
            setError(err instanceof Error ? err.message : "Échec de la transaction de paiement");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleWithdraw = async () => {
        try {
            ...
            const tx = await contractInstance.methods.withdraw().send({ from: blockchainInfo.account });
            const txDetails = await web3.eth.getTransaction(tx.transactionHash);
            setTransactionInfo(txDetails);
            setResult("✅ Retrait effectué avec succès.");
        } catch (err) {
        ...
        <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 sm:p-20 bg-gray-50">
            <main className="flex flex-col gap-10 row-start-2 items-center text-center w-full max-w-2xl">
                ...
                <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-6 text-left max-w-4xl">
                    <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Contrat de Paiement</h1>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-4 flex-col sm:flex-row">
                            <button
                                ...
                                disabled={loading}
                                className="w-full sm:w-1/2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400"
                            >
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="w-full sm:w-1/2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400"
                            >
                ...
                {blockchainInfo.latestBlock && (
                    <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-8 text-left max-w-4xl">
                        ...
                        <div className="bg-gray-50 dark:bg-[#1e1e1e] p-6 rounded-xl space-y-4 border border-gray-200 dark:border-gray-700">
                            <h3 className="text-lg font-semibold text-purple-700 dark:text-purple-400 border-b border-purple-400 dark:border-purple-600 pb-2">Dernier bloc</h3>
