...
export default function Exercise2Page() {
    ...
    const [mode, setMode] = useState<'etherToWei' | 'weiToEther'>('etherToWei');
    ...
    useEffect(() => {
        const fetchBlockchainInfo = async () => {
            try {
                const contractData = (await import('../../contracts/Exercise2.json')) as ContractData;
    ...
    const handleConvert = async () => {
        try {
            ...
            if (mode === 'etherToWei') {
                const etherAmount = web3.utils.toBN(ether);
                const gasEstimate = await contract.methods.etherEnWei(etherAmount).estimateGas({ from: accounts[0] });
                tx = await contract.methods.etherEnWei(etherAmount).send({ from: accounts[0], gas: gasEstimate });
                value = await contract.methods.etherEnWei(etherAmount).call();
            } else {
                const weiAmount = web3.utils.toBN(wei);
                const gasEstimate = await contract.methods.weiEnEther(weiAmount).estimateGas({ from: accounts[0] });
                tx = await contract.methods.weiEnEther(weiAmount).send({ from: accounts[0], gas: gasEstimate });
                value = await contract.methods.weiEnEther(weiAmount).call();
            }
        ...
        <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 sm:p-20 bg-gray-50">
            <main className="flex flex-col gap-10 row-start-2 items-center text-center w-full max-w-2xl">
                ...
                <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-6 text-left max-w-4xl">
                    <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Convertisseur Ether / Wei</h1>

                    <div className="space-y-4">
                        <div className="flex gap-4">
                            <button className={`px-4 py-2 rounded ${mode === 'etherToWei' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`} onClick={() => setMode('etherToWei')}>
                                Ether ➝ Wei
                            </button>
                            <button className={`px-4 py-2 rounded ${mode === 'weiToEther' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`} onClick={() => setMode('weiToEther')}>
                                Wei ➝ Ether
                            </button>
                        </div>
                        {mode === 'etherToWei' ? (
                            <input type="number" placeholder="Saisir le montant en Ether" value={ether} onChange={e => setEther(e.target.value)} className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" disabled={loading} />
                        ) : (
                            <input type="number" placeholder="Saisir le montant en Wei" value={wei} onChange={e => setWei(e.target.value)} className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" disabled={loading} />
                        )}
                        <button onClick={handleConvert} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400">
