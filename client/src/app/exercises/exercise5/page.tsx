...
export default function Exercise5Page() {
    ...
    useEffect(() => {
        const fetchBlockchainInfo = async () => {
            try {
                const contractData = (await import('../../contracts/Exercise5.json')) as ContractData;
    ...
    const handleCheckParity = async () => {
        try {
            ...
            const gasEstimate = await contractInstance.methods.estPair(value).estimateGas({ from: blockchainInfo.account });
            const tx = await contractInstance.methods.estPair(value).send({ from: blockchainInfo.account, gas: gasEstimate });

            const resultValue = await contractInstance.methods.estPair(value).call();
            setResult(resultValue ? '✅ Pair' : '❌ Impair');
        ...
        <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 sm:p-20 bg-gray-50">
            <main className="flex flex-col gap-10 row-start-2 items-center text-center w-full max-w-2xl">
                ...
                <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-6 text-left max-w-4xl">
                    <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Vérifier la parité d’un nombre</h1>
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={handleCheckParity} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400">
                            {loading ? 'Traitement...' : 'Vérifier la parité'}
                        </button>
