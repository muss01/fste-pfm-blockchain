...
export default function Exercise7Page() {
    ...
    useEffect(() => {
        const fetchBlockchainInfo = async () => {
            try {
                const contractData = (await import('../../contracts/Exercise7.json')) as ContractData;
    ...
    const handleCalculateSurface = async () => {
        try {
            ...
            const result = await contractInstance.methods.surface().call();
            const infoMsg = await contractInstance.methods.afficheInfos().call();

            setSurface(result);
            setInfo(infoMsg);
        } catch (err) {
            setError(err instanceof Error ? err.message : "Erreur lors de l'appel à la fonction surface");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 sm:p-20 bg-gray-50">
            <main className="flex flex-col gap-10 row-start-2 items-center text-center w-full max-w-2xl">
                ...
                <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-6 text-left max-w-4xl">
                    <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Informations sur la forme Rectangle</h1>

                    <div className="space-y-4">
                        <button onClick={handleCalculateSurface} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400">
                            {loading ? 'Traitement...' : 'Obtenir Surface & Infos'}
                        </button>
                        {error && <div className="p-3 text-red-700 bg-red-100 rounded-md">{error}</div>}
                        {surface && <div className="p-4 text-green-700 bg-green-100 rounded-md">Surface : {surface}</div>}
                        {info && <div className="p-4 text-blue-700 bg-blue-100 rounded-md">Infos : {info}</div>}
                    </div>
                </div>
