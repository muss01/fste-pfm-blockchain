...
export default function Exercise3Page() {
    ...
    useEffect(() => {
        const fetchBlockchainInfo = async () => {
            try {
                const contractData = (await import('../../contracts/Exercise3.json')) as ContractData;
    ...
    const handleSetMessage = async () => {
        try {
            ...
            const tx = await contractInstance.methods.setMessage(message).send({ from: blockchainInfo.account });
            const txDetails = await web3.eth.getTransaction(tx.transactionHash);

            setTransactionInfo(txDetails);
            setResult('Message mis à jour avec succès');
        } catch (err) {
    ...
    const handleConcat = async () => {
        try {
            ...
            const value = await contractInstance.methods.concatenerAvec(concatInput).call();
            setResult(value);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Erreur lors de la concaténation');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 sm:p-20 bg-gray-50">
            <main className="flex flex-col gap-10 row-start-2 items-center text-center w-full max-w-2xl">
                ...
                <div className="w-full bg-white dark:bg-[#111] p-8 rounded-2xl shadow-md space-y-6 text-left max-w-4xl">
                    <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Gestionnaire de Messages (Chaînes de caractères)</h1>

                    <div className="space-y-4">
                        <input type="text" placeholder="Saisir un message" value={message} onChange={e => setMessage(e.target.value)} className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" disabled={loading} />
                        <button onClick={handleSetMessage} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400">
                            {loading ? 'Traitement...' : 'Définir le message'}
                        </button>

                        <input type="text" placeholder="Concaténer avec le message" value={concatInput} onChange={e => setConcatInput(e.target.value)} className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" disabled={loading} />
                        <button onClick={handleConcat} disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-gray-400">
